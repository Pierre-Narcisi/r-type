...
int main() {
	...
	keymap[KeyKeyboard::KEY_Z] = std::pair<bool, std::function<void(ID)>>(false, [](ID self){
		//ecs::Ecs::getConponentMap<game::AnimationRtype>()[self]. = "up";
	...
	keymap[KeyKeyboard::SPACE] = std::pair<bool, std::function<void(ID)>>(false, [](ID parent){
		if (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - ecs::Ecs::getConponentMap<game::Firerate>()[parent]._lastfire).count() > ecs::Ecs::getConponentMap<game::Firerate>()[parent]._firerate)
		{
			TimedEventAdmin m;
			...
			ecs::Ecs::addComponent<ecs::component::Position>(bullet, ecs::Ecs::getConponentMap<ecs::component::Position>()[parent].x, ecs::Ecs::getConponentMap<ecs::component::Position>()[parent].y);
			ecs::Ecs::addComponent<ecs::component::Hitbox>(bullet, ecs::graphical::Graphic::getTextureSize("Sprite/ClassicBullet/ClassicBullet3.png").x, ecs::graphical::Graphic::getTextureSize("Sprite//ClassicBullet/ClassicBullet3.png").y, true, [parent](ID self, ID other){
					if (other != parent)
					{
						...
						ecs::Ecs::addComponent<ecs::component::Position>(explosion, ecs::Ecs::getConponentMap<ecs::component::Position>()[other].x, ecs::Ecs::getConponentMap<ecs::component::Position>()[other].y);
			...
			ecs::Ecs::getConponentMap<game::Firerate>()[parent]._lastfire = std::chrono::system_clock::now();
		}
	});
