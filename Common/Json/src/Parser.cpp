...
namespace json
{
	...
	Entity Parser::fromString(std::string const &json)
	{
	...
	Entity Parser::fromFile(std::string const &file)
	{
	...
	Entity Parser::fromStream(std::istream &stm)
	{
		Parser	parser(stm);
	...
	Entity Parser::_getObj(void)
	{
		char	c;
		Entity	res(Entity::OBJ);
	...
	Entity Parser::_getArr(void)
	{
		char	c;
		Entity	res(Entity::ARR);
	...
	Entity Parser::_getStr(void)
	{
		return (Entity(__extractStr()));
	}

	Entity Parser::_getBool(void)
	{
		...
		if (!max)
			throw ParserException(_count, _stm);
		return Entity(sValue.str() == "true");
	}

	Entity Parser::_getNull(void)
	{
		std::ostringstream	sValue;
		char			c;
		int			max = sizeof("null");
