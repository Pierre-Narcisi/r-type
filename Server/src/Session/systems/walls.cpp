...
namespace game {namespace system {
		void walls::updateDown(rtype::session::Session &session) {
			if (_bindDown == (unsigned long)-1) {
				...
				ecs::Ecs::addComponent<ecs::component::OnlineComponent>(_bindUp, _bindUp, proto::SpriteId::WALL1);
				float x = _wallList[0].second.x;
				float y = _wallList[0].second.y;
				ecs::Ecs::addComponent<ecs::component::Hitbox>(_bindDown, x, y, true, true/*, [](ID self, ID other){if (!ecs::Ecs::idHasComponents<game::component::wall>(other)){ecs::Ecs::deleteId(other);}}*/);
				...
				float oldSizeX = _wallList[nb].second.x;
		...
		void walls::updateUp(rtype::session::Session &session) {
			if (_bindUp == (unsigned long)-1) {
				...
				ecs::Ecs::addComponent<ecs::component::Hitbox>(_bindUp, x, y, true, true/*, [](ID self, ID other){if (!ecs::Ecs::idHasComponents<game::component::wall>(other)){ecs::Ecs::deleteId(other);}}*/);
